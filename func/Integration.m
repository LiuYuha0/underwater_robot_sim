function [dzita, zita_new, eta_new, q_new] = Integration(KinOnly, eta, DH, DH_r, zita, zita_d, tau, PARAM, Ts)
%
% Integration for both kinematic and dynamic simulations, Euler rule
%
% [dzita, zita_new, eta_new, q_new] = Integration(KinOnly, eta, DH, DH_r, zita, zita_d, tau, PARAM, Ts)
% 
% input:
%       KinOnly  dim 1x1     =1 for kinematic simulation
%       eta      dim 6x1     vehicle position/orientation
%       DH/DH_r  dim nx4     DH table
%       zita     dim 6+nx2   system velocity
%       zita_d   dim 6+nx2   desired system velocity 
%       tau      dim 6+nx2   generalized forces 
%       PARAM    struct      parameters for the dynamic simulation
%       Ts       dim 1x1     sampling time
%
% output:
%       dzita    dim 6+nx2   system acceleration 
%       zita_new dim 6+nx2   system velocity at sample i+1 
...
if (KinOnly==1)
    ...
    dzita = DirectDynamics(eta(4:6),DH,DH_r,zita,tau,PARAM); 
